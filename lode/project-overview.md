...
**Simplified auto-authentication for local use**:
- On first visit, automatically creates Identity, Account, User, and Session
- No sign-up or login required - immediate access to the app
- Default user: "Local User" with email `local@localhost`
- Default workspace: "My Workspace"
- Sessions managed via signed permanent cookies
- Logout immediately re-establishes a new session

**Original system (preserved in models)**:
...
- Magic link passwordless authentication (bypassed)
...
No login required - auto-authenticated on first visit.

Use Chrome MCP tools to interact with the running dev app for UI testing and debugging.

## Coding Style

Please read the separate file `STYLE.md` for guidance on coding style.
